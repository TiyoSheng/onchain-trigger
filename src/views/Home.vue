    ...
    <n-spin size="small" :show="isSpin">
      <n-layout class="layout" :style="{ border: store.state.isIframe ? '1px solid #EEEFF0' : 'none' }">
        ...
        <n-layout class="layout-body" has-sider>
          <n-layout-sider class="layout-sider" content-style="padding: 16px 8px;" :native-scrollbar="false" width="200px"
            bordered v-if="!store.state.isIframe">
            <div class="menu">
              <div class="menu-hd flex-center-sb">Trigger列表
                <n-popover placement="right"
                  style="width: auto;background: rgba(6, 30, 85, 0.9);backdrop-filter: blur(2.5px);color:#FFF"
                  :arrow-style="{ background: 'rgba(6, 30, 85, 0.9)', 'backdrop-filter': 'blur(2.5px)' }">
                  <template #trigger>
                    <svg @click="showModal('trigger')" width="16" height="16" viewBox="0 0 16 16" fill="none"
              ...
              <div class="menu-bd">
                <div class="menu-item flex-center-sb" :class="{ 'menu-item-actived': item.id == store.state.activatedId }"
                  v-for="(item, index) in store.state.triggers" :key="item.i" @click="changeMenu(item.id)">{{ item.name }}
                  <div class="flex-center">
                    <svg @click="editTriggerItem(index)" class="del-icon" t="1683120057177" viewBox="0 0 1024 1024"
                      version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2734" id="mx_n_1683120057178" width="16"
                      height="16">
                      <path
                        ...
                        fill="#4C4F53" p-id="2735"></path>
                    </svg>
                    <n-popconfirm :show-icon="false" @click.stop ref="popconfirmRef">
                      <template #trigger>
                        <svg class="del-icon" width="16" height="16" viewBox="0 0 16 16" fill="none"
                          xmlns="http://www.w3.org/2000/svg">
              ...
              <div class="menu-hd flex-center-sb">Contract列表
                <n-popover placement="right"
                  ...
                  :arrow-style="{ background: 'rgba(6, 30, 85, 0.9)', 'backdrop-filter': 'blur(2.5px)' }">
                  <template #trigger>
                    <svg @click="showModal('contract')" width="16" height="16" viewBox="0 0 16 16" fill="none"
                      xmlns="http://www.w3.org/2000/svg">
                      <path d="M3 8H13" stroke="#9BA0A8" stroke-linecap="round" stroke-linejoin="round" />
                      <path d="M8 13L8 3" stroke="#9BA0A8" stroke-linecap="round" stroke-linejoin="round" />
                    </svg>
              ...
              <div class="menu-bd">
                <div class="menu-item flex-center-sb" v-for="(item, index) in store.state.contracts" :key="item.id"
                  @click="showModal('contract', item)">{{ item.name }}
                  <n-popconfirm :show-icon="false" @click.stop ref="contractPopconfirmRef">
                    <template #trigger>
                      <svg @click.stop class="del-icon" width="16" height="16" viewBox="0 0 16 16" fill="none"
                        xmlns="http://www.w3.org/2000/svg">
                        <path d="M9.33325 6.66663L9.33325 11.3333" stroke="#4C4F53" stroke-linecap="round"
                          stroke-linejoin="round" />
                        <path d="M6.66675 6.66663L6.66675 11.3333" stroke="#4C4F53" stroke-linecap="round"
